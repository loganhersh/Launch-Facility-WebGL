...
///// Slightly modified by Logan Hershberger \\\\\\
...
/**
 * Create a model of a wedge. The wedge produced is the equivalent of a cube cut diagonally
 * across one of the faces.
 * @param side the length of the two shorter sides of the triangular faces
 */
function wedge(side) {
   var coords = [];
   var normals = [];
   ...
   squareface([0,0,0, 0,0,side, side,0,side, side,0,0],[0,-1,0]);
   squareface([0,0,0, 0,side,0, 0,side,side, 0,0,side],[-1,0,0]);
   squareface([0,side,0, 0,side,side, side,0,side, side,0,0],[1,1,0]);
   triangleface([0,0,0, 0,side,0, side,0,0],[0,0,-1]);
   triangleface([0,0,side, 0,side,side, side,0,side], [0,0,1]);
...
///// created using method above and RingGeometry from the THREE.js library /////
/**
 * Creates a model of a 2D ring or partial ring,
 * centered at the origin.
 * @param innerRadius the radius of the hole in the radius; a value of
 *    zero will give a disk rather than a ring.
 * @param outerRadius the radius of the ring, from the center to the
 *    outer edge.  Must be greater than innerRadius.  If not provided,
 *    the default value is 2*innerRadius or is 0.5 if innerRadius is 0.
 * @slices the number of radial subdivisions in the circular approximation
 *    of an annulus.  If not provided, the value will be 32.
 * @param lengthRadians the desired portion of the ring in radians.
 *        Length of 2*Math.PI is a full ring.
 */
function ring2(innerRadius, outerRadius, slices, lengthRadians) {
//    innerRadius = innerRadius;
   outerRadius = outerRadius || innerRadius*2 || 0.5;
   lengthRadians = lengthRadians || 2*Math.PI;
   slices = slices || 32;
   ...
   var radiusStep = outerRadius - innerRadius;
   var i,j;

   for ( j = 0; j <= 1; j ++ ) {
      for ( i = 0; i <= slices; i ++ ) {
         segment = i / slices * lengthRadians;
   ...
   for (j=0; j < 1; j++) {
      var level = j * ( slices + 1 );
      for (i=0; i < slices; i++) {
         segment = i + level;

         var a = segment;
         var b = segment + slices + 1;
         var c = segment + slices + 2;
         var d = segment + 1;
 ...
 * @param portions is the portion in radians of the sphere. Math.PI*2 is the full sphere.
 */
function uvSphere(radius, slices, stacks, portion) {
   radius = radius || 0.5;
   slices = slices || 32;
   stacks = stacks || 16;
   portion = portion || 2*Math.PI;
   var vertexCount = (slices+1)*(stacks+1);
   var vertices = new Float32Array( 3*vertexCount );
   var normals = new Float32Array( 3* vertexCount );
   var texCoords = new Float32Array( 2*vertexCount );
   var indices = new Uint16Array( 2*slices*stacks*3 );
   var du = portion/slices;
 ...
 * @param portion the portion of the cylinder body in radians. Does not work well with
 *   top and bottom enabled.
 */
function uvCylinder(radius, height, slices, noTop, noBottom, portion) {
   radius = radius || 0.5;
